...
// });
const dropZone = document.getElementById('dropZone');
const FileInupt = document.getElementById('image-upload');

let allFiles = []; // Array to store all selected files

dropZone.addEventListener('click', () => {
    FileInupt.click(); // Trigger the file input when the drop zone is clicked
});
...
document.getElementById('image-upload').addEventListener('change', function(event) {
    ...
    for (let i = 0; i < files.length; i++) {
        allFiles.push(files[i]); // Append new files to the array (allow duplicates)
    }

    // Update the preview and file input
    updatePreview();
    updateFileInput();
});
...
// Function to update the file input with the current allFiles array
function updateFileInput() {
    const dataTransfer = new DataTransfer();
    allFiles.forEach(file => dataTransfer.items.add(file)); // Add files to DataTransfer object
