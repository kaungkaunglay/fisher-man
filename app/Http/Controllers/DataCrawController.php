    ...
    public function datashowrating(Request $request)
{
    ...
    // Use a subquery to get the minimum ID for each unique combination
    $subQuery = DataCraw::selectRaw('MIN(id) as id')
                        ->where('date', $today)
                        ->whereNotNull('quantity')
                        ->where('quantity', '>', 0)
                        ...
                        ->groupBy('quantity', 'market', 'fish_type', 'date');

    $data = DataCraw::whereIn('id', $subQuery)
