@extends('includes.layout') @section('style')
    ...
    <div class="sub-category mb-3">
        ...
        <form action="{{ route('sub-category.show', $subCategory->id) }}" method="GET" id="priceRangeForm">
            <div class="range-slider mx-auto">
                <input type="hidden" name="min_price" id="min-price-hidden" />
                <input type="hidden" name="max_price" id="max-price-hidden" />
                <input type="text" class="min-price" id="min-price-input" value="{{ number_format((float) request('min_price', 1)) }}" min="1" max="50000" />
                <h3 class="txt">価格で絞り込み</h3>
                <input type="text" class="max-price" id="max-price-input" value="{{ number_format((float) request('max_price', 10000)) }}" min="1" max="50000" />
        ...
        <div class="sub-category-item">
            <div class="card-list" id="view-list">
                @if ($products && $products->isNotEmpty()) @foreach ($products as $product) @if ($product->status == 'approved')
                <div class="item-card">
        ...
        <div class="row mt-4">
            ...
            <ul class="pagination">
                ...
                @php $start = max(1, $products->currentPage() - 1); $end = min($start + 2, $products->lastPage()); @endphp @for ($i = $start; $i <= $end; $i++)
